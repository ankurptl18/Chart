...
public class PdfHelper {
	...
	public static PdfPTable getTestConditionList(ConditionList cl) {
		...
		while (flag) {
			...
			// int count = 0;
			for (int count = 0 + loop; count < 5 && count < cl.getConditions().size(); count++, loop++) {
				...
				if (count == cl.getConditions().size()) {
					flag = Boolean.FALSE;
				}
			...
			// int count2 = 0;
			for (int count2 = 0 + loop; count2 == 5 && count2 < cl.getConditions().size(); count2++) {
				PdfPCell dataRow = new PdfPCell(new Phrase(cl.getConditions().get(count2).getValue(), normal_10B));
				dataRow.setBorder(PdfPCell.NO_BORDER);
				setRowAlignment(dataRow);
				conditionListTable.addCell(dataRow);
